    ...
    private void OnPuddleSlip(Entity<PuddleComponent> entity, ref SlipEvent args)
    {
        ...
        // after we've had the puddle interact with skin, add back reagents that aren't supposed to stick
            ...
            foreach (var (proto, amt) in splitSol.GetReagentPrototypes(_prototypeManager))
            {
                ...
                {
                    var reagentId = new ProtoId<ReagentPrototype>(proto.ID);
                    solution.AddSolution(splitSol.SplitSolutionWithOnly(splitSol.Volume, reagentId), _prototypeManager);
                }
            }
// </Goobstation>
    }
        ...
        EntityUid? user = null)
    {
        ...
        return TrySplashSpillAt(entity, coordinates, solution.Value.Comp.Solution, out puddleUid, sound, user);
    }
